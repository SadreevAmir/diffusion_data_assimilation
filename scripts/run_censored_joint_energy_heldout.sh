...
cd "$ROOT"
RUN_ID="${CENSORED_JOINT_HELDOUT_ID:?CENSORED_JOINT_HELDOUT_ID is required}"
if [[ ! "$RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]]; then
  echo "unsafe CENSORED_JOINT_HELDOUT_ID" >&2
  exit 2
fi
RESULT_ROOT="/home/autoresearch_results/direct_dynamics_all_hours_v1"
OUTPUT="$RESULT_ROOT/censored_joint_heldout/$RUN_ID"
STATUS_DIR="$RESULT_ROOT/censored_joint_heldout_launches/$RUN_ID"
mkdir -p "$RESULT_ROOT/censored_joint_heldout" \
  "$RESULT_ROOT/censored_joint_heldout_launches" "$RESULT_ROOT/launches"
if ! mkdir -m 700 "$STATUS_DIR"; then
  echo "refusing to reuse censored joint heldout status directory" >&2
  exit 3
fi
...
timeout --signal=TERM --kill-after=120s 7080s python -m assim_lib.censored_joint_energy_heldout \
