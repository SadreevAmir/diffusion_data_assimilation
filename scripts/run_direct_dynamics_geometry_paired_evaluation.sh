...
EVAL_ID="${DIRECT_DYNAMICS_GEOMETRY_EVAL_ID:?DIRECT_DYNAMICS_GEOMETRY_EVAL_ID is required}"
if [[ ! "$EVAL_ID" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]]; then
  echo "unsafe DIRECT_DYNAMICS_GEOMETRY_EVAL_ID" >&2
  exit 2
fi
CONFIG="${DIRECT_DYNAMICS_GEOMETRY_EVAL_CONFIG:-config/experiments/evaluate_direct_dynamics_geometry_objective_ab_v1.json}"
if [[ "$CONFIG" != "config/experiments/evaluate_direct_dynamics_geometry_objective_ab_v1.json" || ! -f "$CONFIG" ]]; then
  echo "unreviewed geometry paired-evaluation config" >&2
  exit 2
fi
...
LAUNCH="$LAUNCH_ROOT/$EVAL_ID"
mkdir -p "$LAUNCH_ROOT"
if [[ -e "$OUTPUT" || -L "$OUTPUT" ]] || ! mkdir -m 700 "$LAUNCH"; then
  echo "refusing to reuse geometry paired-evaluation output or launch" >&2
  exit 3
fi
finish() {
  code=$?
  ...
  printf '{"status":"%s","evaluation_id":"%s","exit_code":%d,"finished_at":"%s","code_commit":"%s"}\n' \
    "$status" "$EVAL_ID" "$code" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$(git rev-parse HEAD)" > "$LAUNCH/status.json"
  exit "$code"
}
trap finish EXIT
printf '{"status":"gpu_admission","evaluation_id":"%s","code_commit":"%s"}\n' \
  "$EVAL_ID" "$(git rev-parse HEAD)" > "$LAUNCH/status.json"
...
printf '{"status":"running","evaluation_id":"%s","started_at":"%s","code_commit":"%s","gpu_uuid":"%s"}\n' \
  "$EVAL_ID" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$(git rev-parse HEAD)" "$GPU_UUID" > "$LAUNCH/status.json"
...
timeout --foreground --signal=TERM --kill-after=120s 14400s \
  python -m assim_lib.direct_dynamics_geometry_paired_evaluation \
