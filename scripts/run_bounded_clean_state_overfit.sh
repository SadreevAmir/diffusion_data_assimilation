...
cd "$ROOT"
EXPERIMENT_ID="${BOUNDED_OVERFIT_ID:?BOUNDED_OVERFIT_ID is required}"
if [[ ! "$EXPERIMENT_ID" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]]; then
  echo "unsafe BOUNDED_OVERFIT_ID" >&2
  exit 2
fi

RESULT_ROOT="/home/autoresearch_results/direct_dynamics_all_hours_v1"
OUTPUT="$RESULT_ROOT/bounded_overfit/$EXPERIMENT_ID"
STATUS_DIR="$RESULT_ROOT/bounded_overfit_launches/$EXPERIMENT_ID"
mkdir -p "$RESULT_ROOT/bounded_overfit" "$RESULT_ROOT/bounded_overfit_launches"
if ! mkdir -m 700 "$STATUS_DIR"; then
  echo "refusing to reuse bounded-overfit status directory" >&2
  exit 3
fi
...
timeout --signal=TERM --kill-after=120s 10680s python -m assim_lib.bounded_clean_state_overfit \
