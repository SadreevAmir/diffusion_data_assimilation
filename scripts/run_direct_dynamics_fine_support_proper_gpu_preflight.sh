...
RUN_ID="${FINE_SUPPORT_PREFLIGHT_RUN_ID:?FINE_SUPPORT_PREFLIGHT_RUN_ID is required}"
if [[ ! "$RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]]; then
  echo "unsafe FINE_SUPPORT_PREFLIGHT_RUN_ID" >&2
  exit 2
fi
CONFIG="${FINE_SUPPORT_PREFLIGHT_CONFIG:-config/experiments/preflight_direct_dynamics_fine_support_proper_gpu_v1.json}"
EXPECTED_CONFIG="config/experiments/preflight_direct_dynamics_fine_support_proper_gpu_v1.json"
if [[ "$CONFIG" != "$EXPECTED_CONFIG" || ! -f "$CONFIG" ]]; then
  echo "unreviewed fine-support GPU preflight config" >&2
  exit 2
fi

OUTPUT_ROOT="/home/autoresearch_results/direct_dynamics_cascade_v2/fine_support_proper_gpu_preflight"
LAUNCH_ROOT="/home/autoresearch_results/direct_dynamics_cascade_v2/fine_support_proper_gpu_preflight_launches"
OUTPUT="$OUTPUT_ROOT/$RUN_ID"
mkdir -p "$LAUNCH_ROOT"
if [[ -e "$OUTPUT" || -L "$OUTPUT" ]]; then
  echo "refusing to reuse fine-support GPU preflight output" >&2
  exit 3
fi
if ! mkdir -m 700 "$LAUNCH_ROOT/$RUN_ID"; then
  echo "refusing to reuse fine-support GPU preflight launch" >&2
  exit 3
fi
...
timeout --foreground --signal=TERM --kill-after=60s 1740s \
  python -m assim_lib.direct_dynamics_fine_support_proper_gpu_preflight \
