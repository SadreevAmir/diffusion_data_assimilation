...
RUN_ID="${GEOMETRY_OBJECTIVE_PREFLIGHT_RUN_ID:?GEOMETRY_OBJECTIVE_PREFLIGHT_RUN_ID is required}"
if [[ ! "$RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]]; then
  echo "unsafe GEOMETRY_OBJECTIVE_PREFLIGHT_RUN_ID" >&2
  exit 2
fi
CONFIG="${GEOMETRY_OBJECTIVE_PREFLIGHT_CONFIG:-config/experiments/preflight_direct_dynamics_geometry_objective_v1.json}"
EXPECTED_CONFIG="config/experiments/preflight_direct_dynamics_geometry_objective_v1.json"
if [[ "$CONFIG" != "$EXPECTED_CONFIG" || ! -f "$CONFIG" ]]; then
  echo "unreviewed geometry-objective preflight config" >&2
  exit 2
fi

OUTPUT_ROOT="/home/autoresearch_results/direct_dynamics_geometry_objective_v1/preflight"
LAUNCH_ROOT="/home/autoresearch_results/direct_dynamics_geometry_objective_v1/preflight_launches"
OUTPUT="$OUTPUT_ROOT/$RUN_ID"
LAUNCH="$LAUNCH_ROOT/$RUN_ID"
mkdir -p "$LAUNCH_ROOT"
if [[ -e "$OUTPUT" || -L "$OUTPUT" ]]; then
  echo "refusing to reuse geometry-objective preflight output" >&2
  exit 3
fi
if ! mkdir -m 700 "$LAUNCH"; then
  echo "refusing to reuse geometry-objective preflight launch" >&2
  exit 3
fi
...
finish() {
  ...
  if [[ "$code" -eq 0 ]]; then
    ...
    printf '{"status":"failed","run_id":"%s","exit_code":%d,"code_commit":"%s"}\n' \
      "$RUN_ID" "$code" "$(git rev-parse HEAD)" > "$LAUNCH/status.json"
  fi
  exit "$code"
}
trap finish EXIT
...
if ! flock -n 9; then
  echo "another project launch owns the shared GPU lock" >&2
  exit 4
fi
...
if [[ "$initial_memory" -le 1024 && "$GPU_MEMORY" -gt 1024 ]]; then
  echo "GPU memory ownership changed during admission" >&2
  exit 9
fi
...
timeout --foreground --signal=TERM --kill-after=60s 3540s \
  python -m assim_lib.direct_dynamics_geometry_preflight \
