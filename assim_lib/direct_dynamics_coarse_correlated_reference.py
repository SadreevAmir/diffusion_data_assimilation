...
class LowRankUnitDiagonalGaussian:
    ...
    def gaussian_score(self, values: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_identity:
            return 0.5 * x.square().mean(dim=1)
    ...
    *,
    ...
    for year_index in range(6):
        ...
        maximum_basis = fit_weighted_second_moment_basis(
            x[keep].reshape(-1, x.shape[-1]), coordinate_fraction,
            max_rank=max(ranks), seed=seed + year_index,
        )
        ...
        for alpha, rank in specs:
            ...
            per_slice = reference.gaussian_score(heldout)
    ...
    return {"candidates": rows, "best": best, "one_se_threshold": threshold, "selected": selected}
