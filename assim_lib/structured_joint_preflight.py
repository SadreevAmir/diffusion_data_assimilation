...
def run_preflight(experiment_path: str | Path, *, build_model: bool = True) -> dict:
    ...
    batches_per_epoch = dataloader_batch_count(
        len(train_dataset), config.train_batch_size, drop_last=True
    )
    steps_per_epoch, planned_optimizer_steps = validate_optimizer_step_budget(
    ...
    for split_name, dataset in (("train", train_dataset), ("valid", valid_dataset)):
        ...
        for index in candidates:
            ...
            background_channels = 3 * (config.trajectory_horizon_days + 1)
            for lead in range(config.trajectory_horizon_days + 1):
            ...
            for lag in range(3):
                mask_channel = background_channels + lag * 5 + 4
                mask = condition[mask_channel : mask_channel + 1]
                _require_binary(mask, f"lag{lag}_mask")
                value = condition[mask_channel - 4 : mask_channel - 2]
                innovation = condition[mask_channel - 2 : mask_channel]
                expanded_mask = mask.expand_as(value)
                if torch.any(value[expanded_mask == 0] != 0) or torch.any(
                    innovation[expanded_mask == 0] != 0
                ):
                    raise ValueError(f"lag{lag} value/innovation leaks outside its mask")
            ...
            lag0_mask = condition[background_channels + 4 : background_channels + 5]
    ...
    forecast_anchor = archive_audit["trajectory_date_audit"]["splits"]["valid"][
        "first_eligible_anchor"
    ]
    forecast_item = M2MForecastDataset.build_structured_forecast_item(
        data_config, forecast_anchor
    )
    if (
    ...
    return {
        ...
        "forecast_metadata_manifest_sha256": archive_audit[
            "forecast_metadata_manifest_sha256"
        ],
        "forecast_audited_content_sha256": archive_audit[
            "forecast_audited_content_sha256"
        ],
        "static_mask_content_sha256": archive_audit["static_mask_provenance"][
            "content_sha256"
        ],
        "sral_content_manifest_sha256": archive_audit["sral_provenance"][
