#!/usr/bin/env python3
"""Fail-closed validation of a controller-visible E4 admission mapping record."""
...
from paper.validate_occurrence_intensity_e4_compact_result import validate_result
...
def validate_mapping_record(record: Mapping[str, Any], contract_bytes: bytes, config_bytes: bytes,
    ...
    if not isinstance(record["upstream_base"], str) or not SAFE_ID.fullmatch(record["upstream_base"]):
        raise ValueError("upstream_base must be one literal safe identifier")
    if record["upstream_decision"] not in {
        "E1_ACCEPTED", "TEMPORAL_MECHANISM_USEFUL", "ATOM_AWARE_TRANSFORM_USEFUL",
    }:
        raise ValueError("upstream_decision must select an accepted frozen predecessor")
    if not isinstance(record["reviewed_mode"], str) or not SAFE_MODE.fullmatch(record["reviewed_mode"]):
    ...
    status, manifest = json.loads(status_bytes), json.loads(manifest_bytes)
    ...
    if record["test_command"] != "python3 -m unittest test.test_occurrence_intensity_e4":
        raise ValueError("test_command must name the unchanged E4 test package")
    if record["test_sentinel"] != "E4_SENTINEL_PASS":
