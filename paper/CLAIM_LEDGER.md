...
| C8 | Finite-ensemble-aware, boundary-aware calibration can improve reliability without destroying spatial dependence. | Active hypothesis, unverified | Motivated by the affine-logit failure mode and classical ensemble postprocessing; joint stop/go gate is predeclared in `RESEARCH_PLAN.md` | The implemented `validation_existing_ensemble_calibration_audit` must now test the frozen candidates with randomized ranks, boundary diagnostics and paired spatial/physical metrics. No result is claimed before that audit completes. |
