...
class UNetTrainer:
    def __init__(self, config: TrainingConfig, model, optimizer, data_loader_train,
        ...
        self._grid = make_normalized_xy_grid().to(self.accelerator.device)
    ...
    def compute_val_loss(self) -> float:
        ...
        with torch.no_grad():
            for batch in self.val_dataloader:
                ...
                grid_coords = self._grid.expand(bs, -1, -1, -1)
    ...
    def save_samples(self, epoch: int):
        """Генерирует один сэмпл и сохраняет PNG с двумя каналами."""
        self.model.eval()
        ...
        sample = channel_denormalize(
            sample,
            self.config.channel_mean,
            self.config.channel_std,
        )
